...
//
//   let myJobSaved = try? newJSONDecoder().decode(MyJobSaved.self, from: jsonData)
//
// To parse values from Alamofire responses:
//
//   Alamofire.request(url).responseMyJobSaved { response in
//     if let myJobSaved = response.result.value {
//       ...
//     }
//   }
...
class MyJobSaved: Codable {
    var total: Int?
    var jobList: [JobListSaved]?
    init(){}
    init(total: Int?, jobList: [JobListSaved]?) {
        self.total = total
        self.jobList = jobList
    }
}

class JobListSaved: Codable {
...
extension DataRequest {
    ...
    @discardableResult
    func responseMyJobSaved(queue: DispatchQueue? = nil, completionHandler: @escaping (DataResponse<MyJobSaved>) -> Void) -> Self {
