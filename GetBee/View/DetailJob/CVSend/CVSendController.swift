...
class CVSendController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblCarrer.text = "\(NSLocalizedString("update", comment: "")) \(DateUtils.convertToShowFormatDate(dateString: self.lstJobApply[indexPath.row].createdDate!))"
        cell.btnStatus.setTitle(StringUtils.genStringStatus(valueStatus: self.lstJobApply[indexPath.row].status!) , for: .normal)
        cell.btnStatus.backgroundColor = StringUtils.genColor(valueStatus: self.lstJobApply[indexPath.row].status!)
        return cell
    }
